...
class ApplicationAgentPrompter(BasicPrompter):
    ...
    def user_prompt_construction(self, request_history: list, action_history: list, prev_plan: str, user_request: str,distance:int, retrieved_docs: str="",control_item: list=[]) -> str:
        """
        ...
        """
        prompt = self.prompt_template["user"].format(action_history=json.dumps(action_history), request_history=json.dumps(request_history), control_item=json.dumps(control_item),
                                             prev_plan=prev_plan, user_request=user_request, retrieved_docs=retrieved_docs,distance=distance)
    ...
    def user_content_construction(self, image_list: list, request_history: list, action_history: list, prev_plan: str, user_request: str,distance:float, retrieved_docs: str="") -> list[dict]:
        ...
        user_content.append({
            "type": "text",
            "text": self.user_prompt_construction(request_history, action_history, prev_plan, user_request,distance, retrieved_docs)
        })
